...
🌐️ *URL*: ${data.url}`
        ...
        case 'sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[❗] Envie um vídeo com uma legenda *!stickerGif* Max 10 segundos exato!', id)
                )
            }
            break
        case 'stickernobg':
       ...
       case 'translate':
            arg = body.trim().split(' ')
            if (arg.length != 2) return client.reply(from, 'Formato errado!', id)
            if (!quotedMsg) return client.reply(from, 'Formato errado', id)
            const quoteText = quotedMsg.type == 'chat' ? quotedMsg.body : quotedMsg.type == 'image' ? quotedMsg.caption : ''
            translate(quoteText, arg[1])
                .then((result) => client.sendText(from, result))
                .catch(() => client.sendText(from, 'Ocorreu um erro!'))            
        case 'brainly':
            if (args.length >= 2){
...
${desc}`)
        ...
        case 'covid':
            ...
            break
        case 'clearall':
            if (!isowner) return client.reply(from, 'somente adm', message.id)
          ...
          case 'bemvindo':
            ...
            if (args.length === 1) return client.reply(from, 'Selecione habilitar ou desativar!', id)
       ...
       case 'cgc':
        ...
        case 'kikar':
            ...
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este comando,   *!kikar* @mencionarMembro', id)
            await client.sendText(from, `Removido do grupo com sucesso!:\n${mentionedJidList.join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
...
🌐️ *URL*: ${url}`
        ...
        case 'adicionar':
            const orang = args[1]
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (args.length === 1) return client.reply(from, 'para usar este recurso,   *!adicionar* 55xxxx...', id)
            if (!isGroupAdmins) return client.reply(from, 'Este comando só pode ser usado por administradores de grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
            try {
